...
public interface AnalyticsRepository extends JpaRepository<com.gr6.SmartCart.common.domain.Order, Long> {
    ...
    @Query(value = "SELECT p.product_id AS productId, p.name AS name, p.base_price AS basePrice, " +
            ...
            "WHERE so.status IN ('PENDING', 'CONFIRMED', 'PREPARING', 'SHIPPING', 'DELIVERED', 'COMPLETED') " +
    ...
    @Query(value = "SELECT DATE(o.created_at) AS reportDate, COALESCE(SUM(so.total_amount), 0) AS revenue, " +
            "COUNT(so.shop_order_id) AS orderCount " +
            "FROM shop_orders so " +
            "JOIN orders o ON so.order_id = o.order_id " +
            "WHERE so.status IN ('PENDING', 'CONFIRMED', 'PREPARING', 'SHIPPING', 'DELIVERED', 'COMPLETED') " +
            "AND DATE(o.created_at) BETWEEN :startDate AND :endDate " +
    ...
    List<DailyRevenueProjection> getAdminDailyRevenue(@Param("startDate") LocalDate startDate,
                                                      @Param("endDate") LocalDate endDate);
    ...
    @Query(value = "SELECT DATE(o.created_at) AS reportDate, COALESCE(SUM(so.total_amount), 0) AS revenue, " +
            ...
            "AND so.status IN ('PENDING', 'CONFIRMED', 'PREPARING', 'SHIPPING', 'DELIVERED', 'COMPLETED') " +
            "AND DATE(o.created_at) BETWEEN :startDate AND :endDate " +
            "GROUP BY DATE(o.created_at) " +
            "ORDER BY DATE(o.created_at) ASC", nativeQuery = true)
    List<DailyRevenueProjection> getShopDailyRevenue(@Param("shopId") Long shopId,
                                                     @Param("startDate") LocalDate startDate,
                                                     @Param("endDate") LocalDate endDate);

    @Query(value = "SELECT COUNT(transaction_id) FROM transactions " +
            "WHERE DATE(created_at) BETWEEN :startDate AND :endDate", nativeQuery = true)
    Long countTotalTransactions(@Param("startDate") LocalDate startDate,
                                @Param("endDate") LocalDate endDate);

    @Query(value = "SELECT COUNT(transaction_id) FROM transactions " +
            "WHERE status = 'SUCCESS' AND DATE(created_at) BETWEEN :startDate AND :endDate", nativeQuery = true)
    Long countSuccessfulTransactions(@Param("startDate") LocalDate startDate,
                                     @Param("endDate") LocalDate endDate);

    @Query(value = "SELECT COUNT(transaction_id) FROM transactions " +
            "WHERE status = 'FAILED' AND DATE(created_at) BETWEEN :startDate AND :endDate", nativeQuery = true)
    Long countFailedTransactions(@Param("startDate") LocalDate startDate,
                                 @Param("endDate") LocalDate endDate);

    @Query(value = "SELECT COALESCE(SUM(amount), 0) FROM transactions " +
            "WHERE status = 'SUCCESS' AND DATE(created_at) BETWEEN :startDate AND :endDate", nativeQuery = true)
    Long sumTotalVolume(@Param("startDate") LocalDate startDate,
                        @Param("endDate") LocalDate endDate);
}
